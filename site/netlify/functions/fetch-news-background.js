...
exports.handler = async (event) => {
  ...
  if (NEWSAPI_KEY) {
    ...
    for (const { q, section } of newsQueries) {
      try {
        ...
        if (r.ok) {
          const data = await r.json();
          const arts = (data.articles||[]).filter(a=>a.title&&a.title!=="[Removed]").slice(0,3).map(a=>({
            title:a.title, deck:a.description||"", url:a.url,
            img:a.urlToImage||"", source:a.source?.name||"News",
            date:a.publishedAt, section,
  ...
  if (ANTHROPIC_KEY && allArticles.length > 0) {
    try {
      const list = allArticles.slice(0,10).map((a,i)=>`${i+1}. [${a.section.toUpperCase()}] "${a.title}" — ${(a.deck||"").substring(0,80)}`).join("\n");
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({
          ...
          max_tokens:1000,
          messages:[{role:"user",content:`You are S.C. Thomas, Editor in Chief of NY Spotlight Report. Write a sharp 1-line kicker and 1-sentence editorial deck for each. NYC-focused, authoritative, direct.\n\nReturn ONLY JSON: [{"index":1,"kicker":"...","editorialDeck":"..."},...]\n\nArticles:\n${list}`}]
        }),
      ...
      if(res.ok){
        ...
        if(Array.isArray(enhancements)) enhancements.forEach(e=>{if(allArticles[e.index-1]){allArticles[e.index-1].kicker=e.kicker;allArticles[e.index-1].editorialDeck=e.editorialDeck;}});
      }
  ...
  console.log(`✅ Stored ${allArticles.length} articles with${ANTHROPIC_KEY?"":" no"} Claude enhancement`);
