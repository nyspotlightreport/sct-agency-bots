#!/usr/bin/env python3
"""
OMEGA BRAIN ΓÇö Self-Learning Master Intelligence v1.0
...
OMEGA_SYSTEM = """You are the Omega Brain ΓÇö the master intelligence of NY Spotlight Report.
...
def generate_strategic_directives(intel: dict, perf: dict) -> list:
    ...
    if not ANTHROPIC:
        return [
            {"department":"Revenue","directive":"Focus all energy on first sale ΓÇö post HN today","priority":1,"expected_impact":"$500-5000 this week"},
...
- Failing workflows: {len(failing_wfs)} ΓÇö {failing_wfs[:5]}
- Healthy workflows: {len(healthy_wfs)}
- Reputation score: {rep.get("score",70)}/100

Top priorities based on data:
1. Revenue: $0 ΓÇö need first sale to unlock momentum
...
def self_improve_underperforming_bots(perf: dict) -> int:
    ...
    for wf_name, stats in worst:
        log.info(f"  ≡ƒöº Analyzing underperformer: {wf_name} ({stats['success_rate']}% success rate)")
...
Return the COMPLETE improved workflow YAML only. No explanation.""",
        ...
        if improved_wf and "name:" in improved_wf:
            ...
            improved += 1
            log.info(f"  Γ£à Improved: {wf_name}")
...
def generate_morning_synthesis(intel: dict, directives: list) -> str:
    ...
    if not ANTHROPIC:
        return f"""OMEGA BRAIN SYNTHESIS ΓÇö {date.today()}
...
ΓÇö Omega Brain"""
    ...
    return claude(
        OMEGA_SYSTEM,
        f"""Write the daily Omega Brain synthesis ΓÇö the master briefing that goes to all departments.
...
def save_synthesis(synthesis: str, directives: list):
    ...
    log.info("Γ£à Synthesis saved to data/omega/daily_synthesis.json")
...
def run():
    ...
    if failing:
        log.warning(f"Underperformers: {len(failing)} ΓÇö {failing[:5]}")
    ...
    if directives and PUSHOVER_KEY:
        ...
        requests.post("https://api.pushover.net/1/messages.json",
            data={"token":PUSHOVER_KEY,"user":PUSHOVER_USR,
                  ...
                  "title":"≡ƒºá Omega Brain Daily Brief"},
            timeout=5)
    
    log.info(f"\nOMEGA BRAIN SYNTHESIS:\n{synthesis}")
    log.info("\nΓ£à Omega Brain complete")
    return directives
