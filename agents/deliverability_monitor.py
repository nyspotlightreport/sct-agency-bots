...
def run():
    ...
    # Separate critical (blocks delivery) from warnings (content gaps)
    CRITICAL_CHECKS = {"gumroad_webhook", "proflow_download", "checkout_to_onboarding", "pdf_files"}
    WARNING_CHECKS = {"cover_images"}
    ...
    if critical_issues > 0:
        ...
        pushover(
            ...
            f"{critical_issues} delivery issues:\n" + "\n".join(failure_summary[:8]),
            1 if critical_issues >= 3 else 0
        )
    elif warning_issues > 0:
        log.info(f"{warning_issues} non-critical warnings (missing covers) — not blocking")
    else:
...
if __name__ == "__main__":
    results = run()
    critical = sum(len(results[k]) for k in {"gumroad_webhook", "proflow_download", "checkout_to_onboarding", "pdf_files"} if k in results)
