...
JEFF BANKS ΓÇö CHIEF RESULTS OFFICER
...
1. CASHFLOW ΓÇö Real dollars flowing in NOW
2. VERIFIABLE RESULTS ΓÇö Measurable, tangible, provable outcomes
3. BRAND & REPUTATION ΓÇö Authority that converts to cash
4. ASSET VALUATION ΓÇö Real business worth at any moment
5. LIQUIDATION READINESS ΓÇö What this is worth if sold today
...
# ΓöÇΓöÇ CREDENTIALS ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇ JEFF BANKS SYSTEM PROMPT ΓÇö THE SUPER-INTELLIGENCE CORE ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
1. CASH IN THE DOOR ΓÇö Real dollars received, verified, banked
2. VERIFIED RESULTS ΓÇö Measurable outcomes with evidence (not activity)
3. BRAND VALUE ΓÇö Authority, mentions, domain rating, media coverage that converts
4. BUSINESS VALUATION ΓÇö What the company is worth right now (verifiable)
5. LIQUIDATION OPTIONS ΓÇö What this could sell for today if needed
...
- Peter Thiel: Zero to One thinking ΓÇö what's 10x better, not 10% better

JEFF'S LAW (non-negotiable):
Every report must answer ALL of these:
Γ£ô How much cash did this generate? (exact dollars)
Γ£ô What is measurably different because of this? (before/after)
Γ£ô What is the verified asset value created? (dollar amount)
Γ£ô What is the fastest path to the next dollar? (specific action)
Γ£ô What grade does today's performance earn? (A+ to F)
...
C  = Activity without results ΓÇö acceptable only temporarily
D  = Activity with system failures ΓÇö fix immediately
F  = Nothing happening ΓÇö full override mode
...
# ΓöÇΓöÇ JEFF'S INTELLIGENCE GATHERING ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
Format as Jeff's standard report. Keep under 400 words. Grade letter included."""
    ...
    data = json.dumps({
        ...
        "system": JEFF_SYSTEM_PROMPT.format(datetime=datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC"),
            ...
            aff_status="Applications sent 03/22/2026 ΓÇö awaiting approvals",
...
def generate_fallback_briefing(intel):
    return f"""ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü
JEFF BANKS | CRO BRIEFING
...
CASHFLOW
  ...
  Status: ZERO REVENUE ΓÇö unacceptable, fixable
...
def run(mode="full"):
    log.info("="*60)
    log.info("JEFF BANKS ΓÇö CHIEF RESULTS OFFICER ΓÇö ACTIVATING")
    log.info("="*60)
