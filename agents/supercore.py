...
try:
    ...
    def claude(system,user,max_tokens=1000,**kw):
        ...
        req=_ur.Request("https://api.anthropic.com/v1/messages",data=data,headers={"Content-Type":"application/json","x-api-key":key,"anthropic-version":"2023-06-01"})
        try:
            with _ur.urlopen(req,timeout=90) as r: return json.loads(r.read())["content"][0]["text"]
        except: return ""
    def claude_json(system,user,max_tokens=1000,**kw):
...
def supa(method,table,data=None,query=""):
    ...
    req=urlreq.Request(f"{SUPA_URL}/rest/v1/{table}{query}",data=body,method=method,headers={"apikey":SUPA_KEY,"Authorization":f"Bearer {SUPA_KEY}","Content-Type":"application/json","Prefer":"return=representation"})
    try:
...
class SuperDirector:
    ...
    def think_json(self,task,max_tokens=1500):
        ...
        r=claude_json(s,task,max_tokens=max_tokens);self._log("think_json",task[:100],json.dumps(r)[:200]);return r
    def fan_out(self,task,n=3,perspectives=None,max_tokens=1000):
        ...
        perspectives=perspectives[:n];self.log.info(f"FAN-OUT: {n} parallel threads...")
        results=[];lock=threading.Lock()
        def _run(p):
            start=time.time()
            out=self.think(f'Analyze from "{p}" perspective:\nTASK: {task}\nBe specific. Rate confidence 0-100.',max_tokens)
            entry={"perspective":p,"output":out,"duration_ms":int((time.time()-start)*1000),"confidence":self._conf(out)}
        ...
        with ThreadPoolExecutor(max_workers=min(n,5)) as ex:
            ...
            for f in as_completed(futs):
                try: f.result()
                except Exception as e: self.log.error(f"Fan-out: {e}")
        self.log.info(f"FAN-OUT complete: {len(results)}/{n}");return results
    def generate_then_rank(self,candidates,criteria="revenue_impact",top_n=1):
        if not candidates: return []
        self.log.info(f"RANK: {len(candidates)} on '{criteria}'...")
        sums="\n".join(f"C{i+1}[{c.get('perspective','?')}]conf:{c.get('confidence',0)}:\n{c.get('output','')[:400]}" for i,c in enumerate(candidates))
        rankings=self.think_json(f"Rank on:{criteria}\nScore 0-100:revenue_impact(40%),feasibility(25%),speed_to_cash(25%),risk(10%)\n{sums}\nReturn JSON array:[{{\"candidate_index\":1,\"final_score\":82,\"rationale\":\"...\"}}]")
    ...
    def chain_of_thought(self,task,steps=3):
        ...
        for i,s in enumerate(subs):
            r=self.think(f"Step {i+1}/{len(subs)}:{s}\nPrevious:{ctx if ctx else '(first)'}")
            results.append({"step":i+1,"task":s,"result":r});ctx+=f"\nStep {i+1}:{r[:300]}"
        synthesis=self.think("Synthesize steps:\n"+chr(10).join(f"Step {r['step']}:{r['result'][:400]}" for r in results))
        return {"steps":results,"synthesis":synthesis}
    def remember(self,category,content,metadata=None):
        supa("POST","director_memory",{"director_id":self.DIRECTOR_ID,"director_name":self.DIRECTOR_NAME,"category":category,"content":content if isinstance(content,str) else json.dumps(content),"metadata":json.dumps(metadata or {}),"session_id":self.sid,"created_at":datetime.utcnow().isoformat()})
    ...
    def recall_all(self,category=None,limit=20):
        q=f"?order=created_at.desc&limit={limit}"
        if category: q+=f"&category=eq.{category}"
        r=supa("GET","director_memory",query=q);return r if isinstance(r,list) else []
    def self_evaluate(self,output,task):
        return self.think_json(f"SELF-EVAL:Task:{task[:500]}\nOutput:{output[:1000]}\nScore 0-100:revenue_relevance,actionability,specificity,completeness,grade A+ to F\nReturn JSON.")
    def execute_full(self,task,parallel_perspectives=None,chain_steps=0,rank_criteria="revenue_impact",delegate_to=None):
        self.log.info(f"FULL PIPELINE:{task[:80]}...");start=time.time()
        result={"director":self.DIRECTOR_NAME,"task":task,"session":self.sid}
        ...
        ev=self.self_evaluate(result["final_output"],task);result["evaluation"]=ev
        result["grade"]=ev.get("grade","C");result["duration_ms"]=int((time.time()-start)*1000)
        ...
        self._log("execute_full",task[:100],f"Grade:{result['grade']}|{result['duration_ms']}ms")
        self.log.info(f"PIPELINE COMPLETE|Grade:{result['grade']}|{result['duration_ms']}ms");return result
    def _log(self,action,inp,out):
        entry={"timestamp":datetime.utcnow().isoformat(),"director":self.DIRECTOR_ID,"action":action,"input":inp,"output":out,"session":self.sid}
        self.action_log.append(entry);supa("POST","director_audit_log",entry)
    def _conf(self,text):
