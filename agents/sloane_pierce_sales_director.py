#!/usr/bin/env python3
"""
Sloane Pierce — VP of Sales & Revenue
NYSR Internal Agency — Sales Department Director
...
Under Sloane's leadership, NYSR Sales converts at 3× industry average
...
def notify(msg, title="Sloane Pierce — Sales"):
...
QUARTERLY_TARGETS = {
    ...
    "pipeline_coverage": 3,    # 3× quota in pipeline
...
PLAYBOOK = {
    "prospecting": {
        ...
        "subject_lines": [
            ...
            "{first_name} — saw you're hiring for {role}",
    ...
    "discovery": {
        "key_questions": [
            ...
            "What would it mean for your business if you could 10× that output?",
    ...
    "objections": {
        ...
        "using competitor": "Interesting — what do you like about them? Most of our clients switched from {competitor} because {differentiator}.",
        ...
        "too busy": "That's exactly why this matters — if you're too busy now, imagine how much faster you'd move with our system handling the content.",
    },
    "closing": {
        ...
        "next_step_close": "The next step is simple — I'll send over the agreement and onboarding link. You can be live by {date}. Does that work?",
...
Be specific. No fluff. Focus on the single biggest risk and the exact next action.""",
        ...
        max_tokens=200
    ) or "• Pull Apollo data on this company
• Send personalized value prop
• Follow up in 3 days"

# ── DAILY SALES BRIEFING ──────────────────────────────────────
def run_daily_briefing():
    """Sloane's daily sales briefing — sent to Chairman every morning."""
    log.info("Sloane Pierce — running daily sales briefing")
    ...
    for c in priority[:3]:
        ...
        coaching.append(f"• {c.get('name','?')} @ {c.get('company','?')} ({c.get('stage','?')}): {note[:100]}")

    total_pipeline = sum(v.get("count",0) for v in stats.values())
    won_count = stats.get("CLOSED_WON",{}).get("count",0)

    briefing = f"""📊 SALES BRIEFING — {datetime.utcnow().strftime('%a %b %d')}
...
TOP DEALS:
{chr(10).join(coaching) if coaching else "• No priority deals — time to load the pipeline!"}

TODAY'S PRIORITY:
→ {'Focus on NEGOTIATION deals — highest close probability' if stats.get('NEGOTIATION',{}).get('count',0) > 0 else 'Move 3 prospects to QUALIFIED today'}"""

    log.info(briefing)
    notify(briefing, "Sloane — Daily Sales Brief")
    return briefing
