#!/usr/bin/env python3
"""
agents/priya_sharma_email_agent.py — FIXED v2
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
FIXES:
...
def classify(sender, subject, snippet):
    """Simple keyword classification — no API call needed for this."""
    ...
    if any(k in text for k in ['client','customer','support','help','question','account']):
        return "CLIENT"
    # GitHub (batch these — never alert individually)
...
def run():
    log.info("Priya Email Agent — starting check")
    ...
    try:
        ...
        for mid in msg_ids[-50:]:  # Max 50 per run
            ...
            if category == "REVENUE":
                revenue_emails.append(f"• {subject[:60]} (from {sender[:40]})")
            elif category in ["URGENT","LEGAL"]:
                urgent_emails.append(f"• {subject[:60]}")
    ...
    # SINGLE batch Pushover — only if high-priority items found
    if revenue_emails:
        pushover(
            f"💰 {len(revenue_emails)} Revenue Email(s)",
    ...
    if urgent_emails:
        pushover(
            f"🚨 {len(urgent_emails)} Urgent Email(s)",
