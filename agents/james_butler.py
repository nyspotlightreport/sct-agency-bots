#!/usr/bin/env python3
"""
James Butler v2.0 — Supreme Personal Concierge
...
def send_email(to: str, subject: str, body: str) -> bool:
    ...
    try:
        ...
        msg["From"]    = f"SC Thomas — NY Spotlight Report <{GMAIL_USER}>"
        msg["To"]      = to
        msg["Subject"] = subject
...
def morning_brief() -> str:
    ...
    email_status = "✅ RESTORED" if status["email_working"] else "⚠️ Needs App Password"
    ...
    if not status["email_working"]:
        lines += [
            "",
            "⚡ One action needed — 60 seconds:",
            "1. Go to: myaccount.google.com/apppasswords",
            "2. Sign in with Seanb041992@gmail.com",
            "3. Generate App Password → name it 'NYSR'",
    ...
    if status["recent_failures"] > 3:
        lines += [
            "",
            f"⚠️ {status['recent_failures']} workflow failures — Guardian is auto-fixing.",
        ]
    
    lines.append("")
    lines.append("— James Butler, Concierge Division")
    
    brief = chr(10).join(lines)
    
    if ANTHROPIC:
        enhanced = claude(BUTLER_VOICE,
            f"Rewrite this morning brief in your voice — formal, efficient, warm. Keep all facts:\n\n{brief}",
            max_tokens=300)
...
def run():
    log.info("James Butler v2.0 — Morning Brief")
    ...
    if PUSHOVER_KEY:
        requests.post("https://api.pushover.net/1/messages.json",
            data={"token":PUSHOVER_KEY,"user":PUSHOVER_USR,
                  "message":brief[:1000],"title":"🎩 James Butler — Morning Brief"},
            timeout=5)
    
    log.info("✅ James Butler complete")
    return brief
